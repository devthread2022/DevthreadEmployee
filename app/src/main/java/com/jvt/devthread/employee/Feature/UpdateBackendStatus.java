...
import com.jvt.devthread.employee.databinding.FragmentUpdateBackendStatusBinding;

import java.text.SimpleDateFormat;
import java.util.Calendar;

public class UpdateBackendStatus extends Fragment {
    private FragmentUpdateBackendStatusBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentUpdateBackendStatusBinding.inflate(inflater,container,false);
        ...
        binding.submit.setOnClickListener(view1 -> {
            ...
            if (work.isEmpty()){
                ...
                databaseReference.child("AssignedProductDevelopers").child(Common.orderId).child("BackendWorkUpdate").push().setValue(workUpdateModel);
                Toast.makeText(getContext(), "Updated successfully..", Toast.LENGTH_SHORT).show();
            }

        });
        binding.upDatedWork.setOnClickListener(view1 -> {
            Common.workType = "Backend";
        ...
        databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    String status = snapshot.child("backendStatus").getValue().toString();
        ...
        binding.completed.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).child("backendStatus")
                            .setValue("Completed");
                }else {
                    databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).child("backendStatus")
                            .setValue("Ongoing");
