...
import com.jvt.devthread.employee.databinding.FragmentUpdateDeploymentStatusBinding;

import java.text.SimpleDateFormat;
import java.util.Calendar;

public class UpdateDeploymentStatus extends Fragment {
    private FragmentUpdateDeploymentStatusBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentUpdateDeploymentStatusBinding.inflate(inflater,container,false);
        ...
        binding.submit.setOnClickListener(view1 -> {
            ...
            if (work.isEmpty()){
                ...
                databaseReference.child("AssignedProductDevelopers").child(Common.orderId).child("DeploymentWorkUpdate").push().setValue(workUpdateModel);
                Toast.makeText(getContext(), "Updated successfully..", Toast.LENGTH_SHORT).show();
            }

        });
        binding.upDatedWork.setOnClickListener(view1 -> {
            Common.workType = "Deployment";
        ...
        databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    String status = snapshot.child("deploymentStatus").getValue().toString();
        ...
        binding.completed.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).child("deploymentStatus")
                            .setValue("Completed");
                }else {
                    databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).child("deploymentStatus")
                            .setValue("Ongoing");
