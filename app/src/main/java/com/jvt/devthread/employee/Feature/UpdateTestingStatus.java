...
import com.jvt.devthread.employee.databinding.FragmentUpdateTestingStatusBinding;

import java.text.SimpleDateFormat;
import java.util.Calendar;

public class UpdateTestingStatus extends Fragment {
    private FragmentUpdateTestingStatusBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentUpdateTestingStatusBinding.inflate(inflater,container,false);
        ...
        binding.submit.setOnClickListener(view1 -> {
            ...
            if (work.isEmpty()){
                ...
                databaseReference.child("AssignedProductDevelopers").child(Common.orderId).child("TestingWorkUpdate").push().setValue(workUpdateModel);
                Toast.makeText(getContext(), "Updated successfully..", Toast.LENGTH_SHORT).show();
            }

        });
        binding.upDatedWork.setOnClickListener(view1 -> {
            Common.workType = "Testing";
        ...
        databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    String status = snapshot.child("testingStatus").getValue().toString();
        ...
        binding.completed.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b){
                    databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).child("testingStatus")
                            .setValue("Completed");
                }else {
                    databaseReferenceUpdate.child("AssignedProductDevelopers").child(Common.orderId).child("testingStatus")
                            .setValue("Ongoing");
